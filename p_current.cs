...
{
    public partial class p_current : UserControl
    {
        public p_current()
        {
            InitializeComponent();
        ...
        public void SetSettings(Settings settings)
        {
            ...
            settings.AS.Range = comboBox_range.SelectedItem.ToString();
            settings.AS.Units = comboBox_units.SelectedItem.ToString();
            settings.AS.Filter = comboBox_filter.SelectedItem.ToString();
            settings.AS.Rate = comboBox_rate.SelectedItem.ToString();
            try
            {
                settings.AS.Delay = int.Parse(textBox_delay.Text);
            }
            catch (Exception ex)
            {
        ...
        public void AmpermeterCommands(AmpermeterSettings ams, Keithley2000 device)
        {

            switch (ams.Range)
            {
                case "Auto":
                    device.Range = ":SENSe:CURRent:DC:RANGe:AUTO ON";
                    break;
                case "3A":
                    device.Range = ":SENSe:CURRent:DC:RANGe 3";
                    break;
                case "1A":
                    device.Range = ":SENSe:CURRent:DC:RANGe 1";
                    break;
                case "100mA":
                    device.Range = ":SENSe:CURRent:DC:RANGe 0.1";
                    break;
                case "10mA":
                    device.Range = ":SENSe:CURRent:DC:RANGe 0.01";
                    break;
            }
            ...
            {
                case "ON":
                    device.Filter = ":SENS:CURRent:DC:AVER:STAT ON";
                    break;
                case "OFF":
                    device.Filter = ":SENS:CURRent:DC:AVER:STAT OFF";
                    break;
            }
            ...
            {
                case "fast":
                    device.Rate = ":SENS:CURRent:DC:NPLC 0.01";
                    break;
                case "medium":
                    device.Rate = ":SENS:CURRent:DC:NPLC 1";
                    break;
                case "slow":
                    device.Rate = ":SENS:CURRent:DC:NPLC 10";
                    break;
            }
        }
        ...
        void Measure()
        {
            ...
            double I = 0;                                     // вычисленное напряжение
            ...
            for (int i = 0; i < settings.MS.MCounts; i++)
            {
                ...
                I = (ReZ / settings.MS.SCounts) * CF.UnitsCoefficient(settings.AS.Units);

                // если есть округление
                if (settings.MS.Round != 0)
                {
                    I = Math.Round(I, settings.MS.Round);
                }
                ...
                if (_filename != null)
                {
                    ...
                    {
                        string _tittle = "#\tDateTime\tTimeSpan (ms)\tCurrent (" + settings.AS.Units + ")";
                ...
                {
                    label_rezult.Text = I.ToString() + " " + settings.AS.Units;
                    label_counts.Text = (i + 1).ToString() + " / " + settings.MS.MCounts.ToString();

                    if (settings.ChS.EnableChart)
                    {
                        CF.DrowChart(chart1, i + 1, I, settings.ChS.Points, settings.ChS.DelPoints);
                    }
                }));
        ...
        private void p_current_Load(object sender, EventArgs e)
        {
            // Задаём функцию измерения - Постоянное напряжение
            device.Function = ":SENS:FUNC 'CURRent:DC'";
            ...
            comboBox_range.SelectedItem = settings.AS.Range.ToString();
            comboBox_units.SelectedItem = settings.AS.Units.ToString();
            comboBox_filter.SelectedItem = settings.AS.Filter.ToString();
            comboBox_rate.SelectedItem = settings.AS.Rate.ToString();
            textBox_delay.Text = settings.AS.Delay.ToString();
        ...
        private void button_start_Click(object sender, EventArgs e)
        {
            ...
            // Применяем настройки
            SetSettings(settings);
            AmpermeterCommands(settings.AS, device);

            // График
            if (settings.ChS.EnableChart)
            {
                chart1.Series[0].Points.Clear();
                chart1.ChartAreas["ChartArea1"].AxisY.Title = "I, " + comboBox_units.SelectedItem.ToString();
            ...
            if (_filename != null)
            {
                string _tittle = "#\tDateTime\tTimeSpan [ms]\tI [" + settings.AS.Units + "]";
                CF.SaveLog(_filename, _tittle, false);
            }
        ...
        private void button_calculate_Click(object sender, EventArgs e)
        {
            ...
            label_rezult.Text = _mean.ToString() + " " + settings.AS.Units;
        }
        ...
        private void button_copy_Click(object sender, EventArgs e)
        {
            if (_row.Count > 0)
            {
                Clipboard.SetText(CF.dataToTableString(_row, "I", settings.AS.Units));
        ...
        private void copyToolStripMenuItem1_Click(object sender, EventArgs e)
        {
            if(chart1.Series[0].Points.Count > 0)
            {
                ...
                string dataRow = $"#\tI [{settings.AS.Units}]";
        ...
        private void button_oneshot_Click(object sender, EventArgs e)
        {
            ...
            Thread oneshotTH = new Thread(() => {
                ...
                Rez = (Rez / settings.MS.SCounts) * CF.UnitsCoefficient(settings.AS.Units);
                ...
                // Show results
                this.Invoke(new ThreadStart(() => {
                    label_rezult.Text = $"{DT.Data} {settings.AS.Units}";
        ...
        private void button_oneshot_copy_Click(object sender, EventArgs e)
        {
            if (oneshotData.Count > 0)
            {
                Clipboard.SetText(CF.dataToTableString(oneshotData, "I", settings.AS.Units));
            }
        }
    }
}
